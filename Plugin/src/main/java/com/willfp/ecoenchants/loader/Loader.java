...
public class Loader {
    ...
    public static void load() {
        ...
        if(!Target.Applicable.ALL.getMaterials().isEmpty()) {
            PLUGIN.getLogger().info("Targets: §aSUCCESS");
        } else {
            PLUGIN.getLogger().info("Targets: §cFAILURE");
        ...
        if(EnchantmentRarity.getAll().size() == 0) {
            PLUGIN.getLogger().severe("§cError loading rarities! Aborting...");
            Bukkit.getPluginManager().disablePlugin(EcoEnchantsPlugin.getInstance());
            return;
        } else {
            PLUGIN.getLogger().info(EnchantmentRarity.getAll().size() + " Rarities Loaded:");
            EnchantmentRarity.getAll().forEach((rarity -> {
                PLUGIN.getLogger().info("- " + rarity.getName() + ": Table Probability=" + rarity.getProbability() + ", Minimum Level=" + rarity.getMinimumLevel() + ", Villager Probability=" + rarity.getVillagerProbability() + ", Loot Probability=" + rarity.getLootProbability());
            }));
        }
        PLUGIN.getLogger().info("");
